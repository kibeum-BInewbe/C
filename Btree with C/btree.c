...
int insert_arec(ele in_rec) {	//하나의 레코드를 삽입  key = 회사명
	...
	do {
		...
		{
			if (strcmp(in_rec.name, curr->rec[i].name) < 0) break; // 비교 시 음수면 좌측 포인터로
			else if (strcmp(in_rec.name, curr->rec[i].name) == 0) return 0; // 동일 레코드 존재하면 중지
		}
		child = curr->ptr[i];

		if (child != NULL) // child가 NULL이 아니면 스택에 쌓고 child를 curr로 지정
		{
			push(curr);
			curr = child;
		}
		...
		if (curr->fill_cnt < MAXK) {
			for (i = 0; i < curr->fill_cnt; i++)
				if (strcmp(in_rec.name, curr->rec[i].name) < 0)
					break; // 삽입할 레코드 키가 작으면 탈출
			for (j = curr->fill_cnt; j > i; j--) {
				curr->ptr[j + 1] = curr->ptr[j]; // 포인터 한칸씩 뒤로 밀기
				curr->rec[j] = curr->rec[j - 1]; // 레코드 한칸씩 뒤로 밀기
			}
			...
			// curr node is full
			for (i = 0; i < MAXK; i++) {			
				if (strcmp(in_rec.name, curr->rec[i].name) < 0) // 키 값이 적으면 탈출 후 삽입
					break;
			}

			bnode.ptr[0] = curr->ptr[0]; // 빅노드에 curr의 첫번째 포인터 대입 
			for (j = 0; j < i; j++) {			 
				bnode.rec[j] = curr->rec[j]; // 빅노드에 curr가 가르키는 rec 순서대로 입력
				bnode.ptr[j + 1] = curr->ptr[j + 1]; // 빅노드에 curr가 가르키는 ptr 순서대로 입력
			}

			bnode.rec[j] = in_rec; // 삽입 레코드 빅노드 삽입 
			bnode.ptr[j + 1] = child; // child 삽입   
			j++;

			while (i < MAXK) {		
				bnode.rec[j] = curr->rec[i]; // 삽입 레코드보다 큰 나머지 레코드 삽입  
				bnode.ptr[j + 1] = curr->ptr[i + 1]; // 이후 포인터 입력 
			...
			curr->fill_cnt = HALFK;
			curr->ptr[i] = bnode.ptr[i]; // bnode 전반부 curr에 삽입하고 fill_cnt 설정 및 마지막 ptr 빅노드 중반부 가리킴

			new_ptr = (nodeptr)malloc(sizeof(node)); // 후반부 삽입을 위해 nodeptr 형태로 node 크기만큼 할당

			for (i = 0; i < HALFK; i++)
			{
				new_ptr->ptr[i] = bnode.ptr[i + 1 + HALFK]; // 전반부와 중반부 넘기고 후반부부터 넣기 위해서 1+HALFK 더함
				new_ptr->rec[i] = bnode.rec[i + 1 + HALFK];
			}

			new_ptr->fill_cnt = HALFK;
			new_ptr->ptr[i] = bnode.ptr[MAXK + 1]; // 빅노드 끝의 포인터를 이용함
			...
			else { // 스택이 empty임 (즉 curr 는 root 노드임.) 새 root 노드를 만들어 curr 의 부모로 함.  
				tptr = (nodeptr)malloc(sizeof(node)); // tptr을 새 root로 만들기 위해 할당
				tptr->rec[0] = in_rec; // 빅노드 중반부 tptr에 삽입
				tptr->ptr[0] = curr; // 전반부가 들어간 curr의 포인터를 root의 첫번째 포인터에 삽입
				tptr->ptr[1] = child; // 후반부가 들아간 child의 포인터를 root 두번째 포인터에 삽입
				tptr->fill_cnt = 1; // fill_cnt 빅노드 중반부 삽입되었으므로 1로 설정
				root = tptr; // 최종으로 tptr을 root로 설정
				total_height++;
...
nodeptr  retrieve(char* skey, int* idx_found) {	//검색 함수 
	...
	do {
		for (i = 0; i < curr->fill_cnt; i++) {
			if (strcmp(skey, curr->rec[i].name) < 0) // 찾는 키값이 현재 키값보다 작으면 탈출
				break;
